...
<head>
    ...
    <style>
        :root {
            ...
            --heading: #111827;
            --grid: #111;
            /* حدود الجدول داكنة مثل الحسابات */
        }
        ...
        <table>
            ...
            <tbody>
                @forelse($sales as $sale)
                    @php
                        // لو ما كانت الأعمدة موجودة نستخدم أسماء افتراضية
                        $usdSell = (float) ($sale->usd_sell ?? 0);
                        $paid = (float) ($sale->amount_paid ?? ($sale->collections_sum_amount ?? 0));
                        $remaining = $usdSell - $paid;
                    @endphp
                    <tr>
                        <td>{{ $sale->id }}</td>
                        <td>{{ $sale->customer->name ?? '—' }}</td>
                        <td>{{ number_format($usdSell, 2) }} {{ $sale->agency->currency ?? 'USD' }}</td>
                        <td>{{ number_format($paid, 2) }} {{ $sale->agency->currency ?? 'USD' }}</td>
                        <td>{{ number_format($remaining, 2) }} {{ $sale->agency->currency ?? 'USD' }}</td>
                        <td>{{ $sale->status ?? '—' }}</td>
                        <td>{{ $sale->created_at ? \Carbon\Carbon::parse($sale->created_at)->format('Y-m-d') : '—' }}
                        </td>
                    </tr>
                @empty
                    <tr>
