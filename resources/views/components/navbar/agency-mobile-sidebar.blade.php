@php
    ...
    $mainAgency = $user->agency->parent_id ? $user->agency->parent : $user->agency;
...
<div class="fixed inset-0 z-50 flex lg:hidden">
    ...
    <!-- القائمة الجانبية -->
    <aside
        class="relative w-64 sm:w-72 max-w-full bg-white shadow-xl h-full flex flex-col rtl:border-l rtl:border-gray-200 ltr:border-r ltr:border-gray-200 overflow-y-auto max-h-screen">
        <!-- رأس القائمة -->
        ...
        <nav class="flex-1 flex flex-col gap-1 px-4 py-4">
            ...
            <!-- الحسابات -->
            @if(Auth::user()->hasRole('agency-admin') || Auth::user()->can('accounts.view') || Auth::user()->can('customers.view') || Auth::user()->can('providers.view')|| Auth::user()->can('quotations.view'))
