<div class="space-y-6">
    ...
    <!-- إعداد: مدة السماح لتعديل المبيعات -->
    @php
        $__mins = (int)($salesEditWindowMinutes ?? 180);
        $__hrs  = intdiv($__mins, 60);
        $__rem  = $__mins % 60;
    @endphp
    <div class="bg-white rounded-xl shadow-md overflow-hidden p-4 border">
        <h3 class="font-bold mb-3" style="color: rgb(var(--primary-700));">مدة السماح لتعديل المبيعات</h3>
        ...
        <div class="flex items-end gap-3">
            <div class="w-44">
                <x-input-field
                    type="number"
                    min="0"
                    wireModel="salesEditWindowMinutes"
                    label="المدة بالدقائق"
                    placeholder="مثال: 300 = 5 ساعات"
                    errorName="salesEditWindowMinutes"
                />
                <p class="text-xs text-gray-500 mt-1">
                    الحالي: {{ $__hrs }} ساعة و {{ $__rem }} دقيقة
                </p>
            </div>

            <x-primary-button wire:click="saveSalesEditWindow">
                حفظ الإعداد
            </x-primary-button>
        </div>
        <p class="text-[11px] text-gray-500 mt-2">
            عند انتهاء المهلة تُمنع عمليات تعديل المبيعات تلقائياً حسب سياسة الوكالة.
        </p>
    </div>
