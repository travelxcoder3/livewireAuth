...
<div class="p-4">
  ...
  <button wire:click="addItem({{ $list->id }})" wire:loading.attr="disabled"
          class="w-full md:w-auto text-white font-bold px-4 py-2 rounded-md text-sm transition duration-300 hover:shadow-lg"
                    ...
                    @foreach ($list->items as $item)
                        <div class="bg-gray-50 rounded px-4 py-2 mb-2 border border-gray-200">
                            ...
                            @foreach ($item->subItems as $sub)
                                <div class="ml-0 sm:ml-6 mt-1 flex justify-between text-xs text-gray-700 gap-2">
                            ...
                            @if ($item->created_by_agency === $agencyId)
                               <div class="flex flex-col md:flex-row items-stretch md:items-center gap-2 mt-2 ml-0 md:ml-6">
  ...
  <button wire:click="addSubItem({{ $item->id }})" wire:loading.attr="disabled"
          class="w-full md:w-auto text-white font-bold px-3 py-2 rounded-md text-sm transition duration-300 hover:shadow"
          style="background: linear-gradient(to right, rgb(var(--primary-500)) 0%, rgb(var(--primary-600)) 100%);">
    إضافة بند فرعي
  </button>
  @endcan
                                ...
                                @error("subItemLabel.$item->id")
                                    <span class="text-red-500 text-xs ml-0 sm:ml-6">{{ $message }}</span>
                                @enderror
                            @endif
                        </div>
                    @endforeach
    ...
    <style>
        ...
        .peer:placeholder-shown+label { display: none; }
        ...
        button[style*="gradient"]:active { transform: translateY(0); }
    </style>
</div>
