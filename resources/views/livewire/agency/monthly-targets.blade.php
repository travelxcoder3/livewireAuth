<div x-data="{ tab: @entangle('tab') }">
    ...
    <div class="bg-white rounded-2xl shadow ring-1 ring-black/5 p-3">
        ...
        <nav class="flex gap-3 sm:gap-6 border-b overflow-x-auto whitespace-nowrap px-1">
            ...
            <button x-on:click="tab='collector'"
                    class="relative py-2 px-2 sm:py-3 sm:px-1 text-[13px] sm:text-sm font-semibold"
                    :class="tab==='collector' ? 'text-[rgb(var(--primary-700))]' : 'text-gray-500'">
                قواعد التحصيل الشهرية
            </button>
            <button x-on:click="tab='debt'"
                    class="relative py-2 px-2 sm:py-3 sm:px-1 text-[13px] sm:text-sm font-semibold"
                    :class="tab==='debt' ? 'text-[rgb(var(--primary-700))]' : 'text-gray-500'">
                سياسة دين الموظف
            </button>
            <button x-on:click="tab='sim'"
                    class="relative py-2 px-2 sm:py-3 sm:px-1 text-[13px] sm:text-sm font-semibold"
                    :class="tab==='sim' ? 'text-[rgb(var(--primary-700))]' : 'text-gray-500'">
                المحاكي
            </button>
        </nav>
        ...
        <div x-show="tab==='emp'" x-cloak class="space-y-4 pt-4">
            <div class="bg-white rounded-xl shadow-md overflow-hidden">
                <div class="overflow-x-auto -mx-1 sm:mx-0">
                    <table class="min-w-full divide-y divide-gray-200 text-[11px] sm:text-xs text-right">
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-2 py-1">
                                    <input type="number" step="0.01" wire:model.defer="rows.{{ $i }}.main_target"
                                        class="w-full sm:w-36 rounded-lg border border-gray-300 px-2 py-1 text-xs" @disabled($r['locked'])>
                                </td>
                                ...
                                <td class="px-2 py-1 hidden md:table-cell">
                                    <input type="number" step="0.01" wire:model.defer="rows.{{ $i }}.override_rate"
                                        class="w-full sm:w-28 rounded-lg border border-gray-300 px-2 py-1 text-xs" @disabled($r['locked'])>
                                </td>
                                <td class="px-2 py-1 text-center hidden sm:table-cell">
                        ...
                        @empty
                            <tr><td colspan="6" class="text-center py-4 text-gray-400">لا توجد بيانات</td></tr>
                        @endforelse
                        </tbody>
                    </table>
                </div>
