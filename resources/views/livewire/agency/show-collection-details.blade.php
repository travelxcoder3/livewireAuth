...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        <h2 class="text-2xl font-bold" style="color: rgb(var(--primary-700)); border-bottom: 2px solid rgba(var(--primary-100), 0.5); padding-bottom: 0.5rem;">
            تفاصيل التحصيل
        </h2>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-md p-6">
        <h3 class="text-lg font-bold mb-4" style="color: rgb(var(--primary-700)); border-bottom: 2px solid rgba(var(--primary-100), 0.5); padding-bottom: 0.5rem;">
            المعلومات الأساسية
        </h3>
    ...
    <div class="bg-white rounded-xl shadow-md p-6">
        ...
        <div class="grid md:grid-cols-4 gap-4 text-sm">
            <div class="flex items-center">
                ...
                <span class="font-medium">{{ optional($sale->collections->last()?->customerType)->label ?? '-' }}</span>
            </div>
            ...
            <div class="flex items-center">
                ...
                <span class="font-medium">{{ optional($sale->collections->last()?->customerResponse)->label ?? '-' }}</span>
            </div>
            <div class="flex items-center">
                <strong class="min-w-[100px] text-gray-500">نوع الارتباط:</strong>
                <span class="font-medium">{{ optional($sale->collections->last()?->customerRelation)->label ?? '-' }}</span>
            </div>
            @endcan
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-md p-6">
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    @forelse($sale->collections as $col)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-2 py-1" style="color: rgb(var(--primary-600)); font-weight: 600;">{{ number_format($col->amount, 2) }}</td>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-4 gap-4 mt-6 text-sm">
            <div class="bg-[rgb(var(--primary-50))] p-3 rounded-lg border border-[rgb(var(--primary-100))]">
                ...
                <span class="font-bold" style="color: rgb(var(--primary-600));">{{ number_format($sale->usd_sell, 2) }}</span>
            </div>
            <div class="bg-[rgb(var(--primary-50))] p-3 rounded-lg border border-[rgb(var(--primary-100))]">
                <strong class="block text-[rgb(var(--primary-700))] mb-1">المدفوع:</strong>
                <span class="font-bold" style="color: rgb(var(--primary-600));">{{ number_format($amountReceived, 2) }}</span>
            </div>
            <div class="bg-[rgb(var(--primary-50))] p-3 rounded-lg border border-[rgb(var(--primary-100))]">
                <strong class="block text-[rgb(var(--primary-700))] mb-1">المتبقي:</strong>
                <span class="font-bold" style="color: rgb(var(--primary-600));">{{ number_format($remainingAmount, 2) }}</span>
            </div>
            <div class="bg-[rgb(var(--primary-50))] p-3 rounded-lg border border-[rgb(var(--primary-100))]">
        ...
        <div class="bg-white rounded-xl shadow-xl w-full max-w-md mx-4 p-6 relative transform transition-all duration-300">
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                ...
                <div class="text-sm">
                    ...
                    <input type="text" readonly class="w-full mt-1 border rounded-lg px-3 py-2 bg-[rgb(var(--primary-50))]"
                ...
                <div class="text-sm">
                    ...
                    <input type="text" readonly class="w-full mt-1 border rounded-lg px-3 py-2 bg-[rgb(var(--primary-50))] text-right font-bold"
            ...
            <div class="text-sm mb-4">
                ...
                <input type="number"
                    wire:model.defer="payRemainingNow"
                    max="{{ $remainingAmount }}"
