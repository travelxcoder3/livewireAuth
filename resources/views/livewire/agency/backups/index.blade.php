...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-xl shadow p-4 flex flex-wrap gap-3 items-center">
        ...
        <form id="restore-upload-form" method="POST"
            ...
            <x-primary-button id="restoreUploadBtn" type="submit" 
                :gradient="false" color="#8d8a8aff" textColor="white" icon='<i class="fas fa-upload"></i>'>
                استعادة من ملف مرفوع
            </x-primary-button>
    ...
    {{-- cards in a responsive grid (جنب جنب) --}}
            ...
            @forelse($backups as $b)
                <div class="border rounded-xl p-4 flex flex-col justify-between">
                    ...
                    <form method="POST"
                        ...
                        @csrf
                        <x-primary-button type="submit" class="px-3 py-1" icon='<i class="fas fa-sync"></i>'>
                            استعادة
                        </x-primary-button>
                    </form>
        ...
        <div class="fixed inset-0 z-50 bg-black/10 flex items-center justify-center backdrop-blur-sm">
            <div class="bg-white rounded-xl shadow-xl w-full max-w-md mx-4 p-6 relative transform transition-all duration-300">
                <button @click="show=false"
                        class="absolute top-3 left-3 text-gray-400 hover:text-red-500 text-xl font-bold">&times;
                </button>
