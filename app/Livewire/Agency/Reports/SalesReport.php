...
public function exportToExcel()
{
    ...
    foreach ($sales as $s) {
        ...
        if (in_array($s->status, ['Refund-Full','Refund-Partial'])) {
            $paidFromSales = 0; $paidFromCollections = 0;
        }
...
protected function prepareReportData()
{
    ...
    $query = Sale::query()
        ...
        ->with([
            'user:id,name','provider:id,name','service:id,label','customer:id,name','duplicatedBy:id,name',
        ])
    ...
    $columns = array_values(array_filter(\App\Tables\SalesTable::columns(true, true),
        fn($c)=>($c['key'] ?? '') !== 'actions'));
